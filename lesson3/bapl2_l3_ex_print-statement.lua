...
local function assignmentNode(id, exp)
  return {
    tag = "assignment",
    id = id,
    exp = exp
...
local scientific    = ((decimalFloat + decimalInt) * S("eE") * decimalInt) / numNode
...
local g = P({"statementsOrExps",
  statementsOrExps  = statementOrExp * ((SC * statementsOrExps) + SC)^-1 / sequenceNode,

  block             = OB * statementsOrExps * SC^-1 * CB,

  statementOrExp    = block
                    + (ID * assignment * exp4 / assignmentNode)
                    + (ret * exp4 / returnNode)
                    + (prt * exp4 / printNode)
                    + exp4,
...
function Test:testStatementSequenceBlock()
  local cases = {
    ...
    { input = "result = k10 + 10", result = 20 },
    { input = "result = k10; result2 = 20", result = 10, result2 = 20 },
    { input = "{ result = k10 }", result = 10 },
    { input = "{ result = k10; }", result = 10 },
    { input = "{ result = k10; result2 = 20; }", result = 10, result2 = 20 },
    { input = "{ result = k10;; }", result = 10 },
  }
  ...
  for _, case in ipairs(cases) do
    ...
    lu.assertEquals(memory.result, case.result)
    if case.result2 then
      lu.assertEquals(memory.result2, case.result2)
    end
  end
end
