...
local DEBUG = false
local TEST = false
local TEST_ARITHMETIC = false
local TEST_VARIABLE_EXPRESSION = false
local TEST_STATEMENT_SEQUENCE_BLOCK = false
local TEST_RETURN = false
local TEST_PRINT = false
local TEST_UNDEFINED_VARIABLE = false
local TEST_SYNTAX_ERROR_PRINT = false
local TEST_COMMENTS_BLOCK_COMMENTS = false
local TEST_NOT_EXPRESSION = false
local TEST_IF_ELSE_STATEMENT = false
local TEST_WHILE_STATEMENT = false
local TEST_LOGICAL_OPERATOR = false
...
local assignmentNode = node("assignment", "lhs", "exp")
local indexedNode = node("indexed", "array", "index")
local newArrayNode = node("new", "size")
...
local whileNode = node("while1", "cond", "body")
...
local g = P({"program",
  ...
  exp0              = Rw("new") * T("[") * expTop * T("]") / newArrayNode
                    + numeral
                    + (T("(") * expTop * T(")"))
                    + lhs,

  lhs               = var * T("[") * expTop * T("]") / indexedNode
                    + var,
...
function Test:testVariableExpression()
  ...
  if TEST_VARIABLE_EXPRESSION then
    for _, case in ipairs(cases) do
      local parsed = parse(case.input)
      local code = compile(parsed)
...
function Test:testStatementSequenceBlock()
  ...
  if TEST_STATEMENT_SEQUENCE_BLOCK then
    for _, case in ipairs(cases) do
      local parsed = parse(case.input)
      local code = compile(parsed)
...
function Test:testUndefinedVariables()
  ...
  if TEST_UNDEFINED_VARIABLE then
    for _, case in ipairs(cases) do
      local parsed = parse(case.input)
...
function Test:testSyntaxErrorPrint()
  ...
  if TEST_SYNTAX_ERROR_PRINT then
    for _, case in ipairs(cases) do
      DEBUG = true
      local parsed = parse(case.input)
      DEBUG = false
...
function Test:testCommentsBlockComments()
  ...
  if TEST_COMMENTS_BLOCK_COMMENTS then
    for _, case in ipairs(cases) do
      local parsed = parse(case.input)
      local code = compile(parsed)
...
function Test:testIfElseStatement()
  local cases = {
    { input = [[
      a = 1;
      if a { b = 5; };
  ...
  if TEST_IF_ELSE_STATEMENT then
    for _, case in ipairs(cases) do
      local parsed = parse(case.input)
      local code = compile(parsed)
...
function Test:testWhileStatement()
  local cases = {
    { input = [[
      n = 6;
      r = 1;
...
-- TEST_VARIABLE_EXPRESSION = true
-- TEST_STATEMENT_SEQUENCE_BLOCK = true
-- TEST_RETURN = true
-- TEST_PRINT = true
-- TEST_UNDEFINED_VARIABLE = true
-- TEST_SYNTAX_ERROR_PRINT = true
-- TEST_COMMENTS_BLOCK_COMMENTS = true
-- TEST_NOT_EXPRESSION = true
-- TEST_IF_ELSE_STATEMENT = true
-- TEST_WHILE_STATEMENT = true
-- TEST_ARRAY = true
-- TEST_LOGICAL_OPERATOR = true
