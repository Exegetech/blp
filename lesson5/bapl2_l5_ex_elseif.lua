...
local g = P({"program",
  ...
  ifStat            = Rw("if") * exp5 * block
                    * (Rw("elseif") * exp5 * block)^0
                    * (Rw("else") * block)^-1
                    / foldIfNode,

  assignStat        = ID * T("=") * exp5 / assignmentNode,

  returnStat        = Rw("return") * exp5 / returnNode,

  printStat         = printOp * exp5 / printNode,
...
local function codeJump(state, op)
  addCode(state, op)
  addCode(state, 0)
...
local function codeStatement(state, ast)
  ...
  elseif ast.tag == "if1" then
    ...
    local jump = codeJump(state, "jumpZ")
    codeStatement(state, ast.th)

    if ast.el == nil then
      fixJump2Here(state, jump)
    else
      local jump2 = codeJump(state, "jump")
      fixJump2Here(state, jump)
